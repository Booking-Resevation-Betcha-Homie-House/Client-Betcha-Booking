    ...
    try {
       ...
       pendingContainer.innerHTML = '';
        ...
        bookings.forEach(booking => {
            ...
            console.log(booking.UnitId.unitName, booking.UnitId.location, booking.CheckIn, booking.CheckOut, booking.Status);
            if (booking.Status === 'Pending' || booking.Status === 'Fully-Paid' || booking.Status ==='Arrived'){
                console.log(booking.Status + 'pending');
                const card = document.createElement('div');
                card.className = 'd-flex align-items-center gap-5 my-bookings';
                card.id = `${booking.Status}-bookings`;
            ...
            if (booking.Status === 'Successful' || booking.Status === 'Did Not Arrive') {
                console.log(booking.Status + 'succcessful');
                ...
                console.log(booking.Reference);
                card.onclick = () => {
                    window.location.href = `../LoggedIn/My-Bookings-View.html?id=${booking.Reference}`; 
                };
                card.appendChild(viewIcon);
