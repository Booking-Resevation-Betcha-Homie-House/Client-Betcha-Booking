(function ($) {
    ...
    $(document).ready(() => {
        ...
        const calendars = [
            ...
            { el: $("#calendar_second"), month: month + 1, year: year + (month === 11 ? 1 : 0) }
        ];
        ...
        const renderCalendar = (calendar) => {
			...
			for (let day = 1; day <= daysInMonth; day++) {
				...
				// Adjust date for consistent local representation
		...
		const renderCalendars = () => {
			...
			calendars.forEach(renderCalendar); // Render each calendar independently
			updateSelection(); // Ensure the selection persists across both calendars
		};
		...
		const handleClick = (e) => {
			...
			// If clicking on the same date as the start date, reset the selection
			...
			// If no start date has been set or if there is an end date already, set the start date
			if (!selectedDates.start || (selectedDates.start && selectedDates.end)) {
				selectedDates = { start: clickedDate, end: null };
			} else {
				// If there's already a start date and we are setting the end date
				selectedDates.end = clickedDate;
			}
		
			// Determine start and end dates in the correct order
			const start = new Date(selectedDates.start);
			...
			// Validate the date range between start and end
			for (let d = new Date(rangeStart); d <= rangeEnd; d.setDate(d.getDate() + 1)) {
				...
				if (disabledDates.includes(formattedDate)) {
					...
					selectedDates = { start: null, end: null };
					updateSelection();
					// Reset input fields when invalid date is selected
					$("#input-start-date").val("");
					$("#input-end-date").val("");
					return;
				}
			...
			// Update input fields with the selected dates
			$("#input-start-date").val(start.toISOString().split("T")[0]);
		...
		const switchCalendar = (direction) => {
			...
			console.log("Left button:", $(".switch-month.left").length);
			console.log("Right button:", $(".switch-month.right").length);
			...
			renderCalendars(); // Re-render both calendars after switching
		};
